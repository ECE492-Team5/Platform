...
void generateJSON(int channel, int value) {
    ...
    json_object *j_sensor_value_double = json_object_new_double(value);
    //Initialize new libjson String object (for Date)
    json_object *j_date_string = json_object_new_string(date_buffer);
    //Initialize new libjson String object (for Unit)
    json_object *j_unit_string = json_object_new_string("Amp");

    //Add the INT and String objects to JSON object 
    json_object_object_add(j_sensor_obj, "Channel", j_sensor_channel_int);
    json_object_object_add(j_sensor_obj, "Voltage", j_sensor_value_double);
    json_object_object_add(j_sensor_obj, "Date", j_date_string);
    json_object_object_add(j_sensor_obj, "Unit", j_unit_string);

    //Generate path buffers (temp has a ~)
    snprintf(path_buffer_temp, 30, "./sensors/sensor_%d~.json", channel);
    snprintf(path_buffer, 30, "./sensors/sensor_%d.json", channel);
